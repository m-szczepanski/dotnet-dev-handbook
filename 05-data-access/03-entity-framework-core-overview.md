...
1. **N+1 Query Problem**:
   - When querying related data (e.g., products with their categories), EF Core may issue multiple queries to fetch related entities. Use `.Include()` or lazy loading to optimize this.
...
- **Optimize Queries with `.Include()` or Lazy Loading**: Always consider the N+1 query problem when fetching related data. Use `.Include()` to eagerly load relationships or enable lazy loading if appropriate.
...
4. **Watch Out for N+1 Queries**: Be mindful of fetching related data efficiently using `.Include()` or lazy loading.
